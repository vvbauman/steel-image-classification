...
objective_function= 'categorical_crossentropy' # Wang 2021 uses weighted cross entropy for their binary classifier. they also used focal loss
...
# note: labels are such that ResNet50 model will predict defect class, not just whether a defect is present
train_data= np.load(open('bottleneck_features_train.npy', 'rb'))
train_labels= to_categorical(np.array([0] * 1266 + [1] * 1261 + [2] * 1272 + [3] * 1260 + [4] * 1250)) # data wasn't shuffled when bottleneck features were created
validation_data= np.load(open('bottleneck_features_validation.npy', 'rb'))
validation_labels= to_categorical(np.array([0] * 141 + [1] * 140 + [2] * 141 + [3] * 140 + [4] * 139))
...
test_labels= np.array([0] * 941 + [1] * 769 + [2] * 195 + [3] * 4759 + [4] * 516)
